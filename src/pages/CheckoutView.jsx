        ...
        {step === 1 ? (
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <button
                  ...
                  className="text-xs font-black text-[#FF0055] underline cursor-pointer disabled:text-gray-400 disabled:no-underline"
                >
          ...
          <div className="space-y-6">
            ...
            <div className="bg-[#FFF8E7] border-3 border-[#1a1c1c] p-5 rounded-2xl space-y-2 text-xs font-bold shadow-[4px_4px_0px_0px_#FFD23F]">
              <div className="flex justify-between"><span>SUBTOTAL ({cart.reduce((a,c) => a + c.quantity, 0)} ITEMS)</span><span>{money(subtotal)}</span></div>
              {appliedCoupon && (
              ...
              <div className="flex justify-between"><span>ANYWHERE DOOR DISPATCH</span><span>{money(deliveryFee)}</span></div>
              <div className="flex justify-between"><span>TAX</span><span>{money(summary?.tax || 0)}</span></div>
              {platformFee > 0 && <div className="flex justify-between"><span>PLATFORM FEE</span><span>{money(platformFee)}</span></div>}
            ...
            <div className="flex gap-4 pt-4 border-t-2 border-dashed border-[#1a1c1c]">
              ...
              >
                {isPlacingOrder
                  ? (paymentIsOnline ? 'OPENING RAZORPAY...' : 'PLACING ORDER...')
                  : `${paymentIsOnline ? 'PROCEED TO RAZORPAY' : 'PLACE CASH ORDER'} • ${money(total)}`}
              </button>
            </div>
          </div>
        )}
