...
export default function CheckoutView({
  ...
  return (
    <div className="w-full py-8 flex flex-col items-center relative z-10 text-[#1a1c1c] max-w-5xl mx-auto px-4 font-body-md">
      ...
      <div className="w-full bg-gradient-to-r from-[#0B132B] via-[#1C2541] to-[#0B132B] text-white border-4 border-[#1a1c1c] shadow-[10px_10px_0px_0px_#FF6B00] rounded-[2rem] p-6 md:p-8 text-center mb-8 relative overflow-hidden">
        ...
        <span className="bg-[#FF6B00] text-white border-2 border-black px-4 py-1 rounded-full text-xs font-headline-md font-black uppercase tracking-wider inline-block mb-3 shadow-[3px_3px_0px_0px_#111111] rotate-[-1deg]">
        ...
        <p className="font-body-lg text-xs md:text-sm font-bold text-gray-200 mt-2">
      ...
      <div className="w-full bg-white border-4 border-[#1a1c1c] shadow-[10px_10px_0px_0px_#111111] p-6 md:p-8 rounded-[2rem] space-y-6 relative">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 border-b-4 border-[#1a1c1c] pb-4 gap-3 font-headline-md text-xs md:text-sm uppercase">
          <button
            type="button"
            onClick={() => setStep(1)}
            className={`border-3 border-[#1a1c1c] rounded-2xl px-5 py-3.5 transition-all text-left font-headline-md font-black flex items-center justify-between cursor-pointer ${
              step === 1
          ...
          >
            ...
            {addressIsComplete && <span className="bg-white text-green-700 px-2 py-0.5 rounded text-[10px] border border-black font-headline-md font-black">READY</span>}
          </button>
          ...
          >
            ...
            {selectedPaymentId && <span className="bg-white text-blue-800 px-2 py-0.5 rounded text-[10px] border border-black font-headline-md font-black">SELECTED</span>}
          </button>
        </div>
        ...
        {step === 1 ? (
          ...
          <div className="space-y-6">
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="font-headline-lg text-lg md:text-xl font-black uppercase flex items-center gap-2">
                ...
                <button
                  ...
                  className="font-headline-md text-xs font-black text-[#FF6B00] underline cursor-pointer disabled:text-gray-400 disabled:no-underline"
                >
                ...
                <div className="space-y-3">
                  {addresses.map((addr) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-headline-md font-bold text-xs uppercase text-[#1a1c1c]">{addr.title} ({addr.tag})</span>
                            <span className="font-body-md text-[11px] font-semibold text-gray-500">[{addr.recipientName} • {addr.phone}]</span>
                          </div>
                          <span className="font-body-md text-xs font-bold text-gray-700 block mt-0.5">{addr.addressLine1 || addr.address}</span>
                      ...
                      {addr.default && (
                        <span className="bg-[#10B981] text-white text-[9px] font-headline-md font-black px-2 py-0.5 rounded-full uppercase border border-black shadow-[2px_2px_0px_0px_#000]">
                          PRIMARY BASE ⭐️
                        </span>
                      )}
                    </label>
            ...
            <div className="space-y-3 pt-2">
              <h3 className="font-headline-lg text-lg font-black uppercase flex items-center gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {['Leave at the door', 'Do not ring the bell', 'Call when you arrive'].map((note, i) => (
                  <label
                    key={i}
                    className={`flex items-center gap-3 p-3.5 border-3 border-[#1a1c1c] rounded-2xl font-headline-md text-xs md:text-sm font-bold cursor-pointer transition-all ${
            ...
            <button
              ...
              className="w-full py-4 bg-[#FF6B00] text-white border-4 border-[#1a1c1c] shadow-[6px_6px_0px_0px_#111111] font-headline-md font-black text-sm md:text-base uppercase rounded-2xl cursor-pointer hover:bg-[#e05e00] transition-all flex items-center justify-center gap-2 mt-4"
            >
          ...
          <div className="space-y-6">
            <div>
              <h3 className="font-headline-lg text-lg md:text-xl font-black uppercase flex items-center gap-2 mb-3">
                <span>⚡</span> SELECT KI PAYMENT MANIFEST
              </h3>

              <div className="space-y-3 font-headline-md text-xs md:text-sm font-bold">
                {paymentMethods.map((pm) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <span className="font-headline-md font-bold text-xs md:text-sm uppercase text-[#1a1c1c]">
                        {pm.type} ({pm.details})
                      </span>
                    </div>
                    {pm.primary && (
                      <span className="bg-[#00F0FF] text-[#1a1c1c] text-[9px] font-headline-md font-black px-2 py-0.5 rounded-full uppercase border border-black shadow-[2px_2px_0px_0px_#000]">
                        DEFAULT METHOD ⚡
                      </span>
                    )}
                  </label>
            ...
            <div className="rounded-2xl border-3 border-[#1a1c1c] bg-[#FFF8E7] p-4 shadow-[4px_4px_0px_0px_#FFD23F]">
              <label className="mb-2 block font-headline-md text-xs md:text-sm font-black uppercase text-[#1a1c1c]">
              ...
              <input
                ...
                className="w-full rounded-xl border-2 border-[#1a1c1c] bg-white p-2.5 font-headline-md font-black text-sm text-[#1a1c1c]"
                placeholder="0"
              />
              <p className="mt-1 font-body-md text-xs font-semibold text-gray-600">Points are converted to discount energy upon final order confirmation.</p>
            </div>

            {/* Shenron Wish Summary (Manifest) */}
            <div className="bg-[#FFF8E7] border-4 border-[#1a1c1c] p-6 rounded-2xl space-y-2.5 font-headline-md text-xs md:text-sm font-bold shadow-[6px_6px_0px_0px_#FF6B00]">
              <div className="flex justify-between items-center font-headline-lg text-base font-black border-b-2 border-dashed border-[#1a1c1c] pb-2 mb-2">
                <span>SHENRON BATTLE MANIFEST</span>
                <span className="bg-[#FF6B00] text-white px-2 py-0.5 rounded text-[10px] uppercase font-headline-md font-black">{cart.reduce((a,c) => a + c.quantity, 0)} ITEMS</span>
              </div>
              ...
              <div className="flex justify-between font-headline-lg text-xl md:text-2xl font-black border-t-4 border-[#1a1c1c] pt-3 text-[#FF6B00]">
            ...
            <div className="flex gap-4 pt-4 border-t-4 border-dashed border-[#1a1c1c]">
              <button
                type="button"
                onClick={() => setStep(1)}
                className="px-6 py-4 bg-gray-200 border-4 border-[#1a1c1c] font-headline-md font-black text-xs uppercase rounded-2xl cursor-pointer hover:bg-gray-300 transition-all"
              >
                BACK
              </button>
              ...
              <button
                ...
                className={`flex-1 py-4 border-4 border-[#1a1c1c] shadow-[6px_6px_0px_0px_#111111] font-headline-md font-black text-sm md:text-base uppercase rounded-2xl cursor-pointer transition-all flex items-center justify-center gap-2 ${
                  isPlacingOrder
